...
def model_builder_xrfm(params: Dict[str, Any]) -> xRFM:
    """
    Build an xRFM regressor from a parameter dictionary.
    """
    # ---- leaf params ----
...
def fit_fn_xrfm(model, X_tr, y_tr, X_val, y_val):
    ...
    Includes duplicated-target workaround for the current xRFM regression bug.
    """
    ...
    # Workaround: duplicate target column to satisfy xRFM regression shape bug
...
def predict_fn_xrfm(model, X):
    """
    Predict using xRFM and collapse duplicated regression output if needed.
    """
...
def build_xrfm_preprocessor(X_train, target_col: Optional[str] = None) -> ColumnTransformer:
    """
    Build preprocessing pipeline for xRFM.
...
def tune_xrfm_regressor(
    ...
    """
    Preprocess train/test data, tune xRFM using bayes_tune_model,
